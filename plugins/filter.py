...
from Medoy.dB.filter_db import add_filter, get_filter, list_filter, rem_filter
from Medoy.fns.tools import create_tl_btn, format_btn, get_msg_button

from . import events, get_string, mediainfo, udB, Medoy-bot, kazu_cmd
from ._inline import something


@Medoy_cmd(pattern="addfilter( (.*)|$)")
async def af(e):
    ...
    Medoy-bot.add_handler(filter_func, events.NewMessage())


@Medoy_cmd(pattern="remfilter( (.*)|$)")
async def rf(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    chat = e.chat_id
    if not wrd:
