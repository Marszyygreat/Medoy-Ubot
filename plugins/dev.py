...
from Medoy import _ignore_eval
...
@Medoy_cmd(pattern="bash", fullsudo=False, only_devs=False)
async def _(event):
...
@Medoy_cmd(pattern="eval", fullsudo=False, only_devs=False)
async def _(event):
    try:
        cmd = event.text.split(maxsplit=1)[1]
    except IndexError:
...
@Medoy_cmd(pattern="cpp", only_devs=False)
async def doie(e):
    ...
    open("cpp-Medoy.cpp", "w").write(match)
    m = await bash("g++ -o CppKazu cpp-Medoy.cpp")
    o_cpp = f"• **Eval-Cpp**\n`{match}`"
    if m[1]:
        o_cpp += f"\n\n**• Error :**\n`{m[1]}`"
        if len(o_cpp) > 3000:
