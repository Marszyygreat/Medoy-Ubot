...
async def crbn(event):
    ...
    xx = await Carbon(code=code, file_name="Medoy_carbon", backgroundColor=col)
    await xxxx.delete()
    await event.reply(
...
@Medoy_cmd(pattern="rayso")
async def pass_on(kaz):
    spli = doy.text.split()
    theme, dark, title, text = None, True, get_display_name(doy.chat), None
    if len(spli) > 2:
        ...
        if spli[1] in RaySoTheme:
            ...
            try:
                text = doy.text.split(maxsplit=1)[1]
            except IndexError:
                pass
    if not theme:
