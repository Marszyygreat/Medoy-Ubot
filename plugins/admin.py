...
from Medoy.fns.admins import ban_time
...
@Medoy_cmd(pattern="pin$", manager=True, require="pin_messages", fullsudo=True)
async def pin(msg):
    if not msg.is_reply:
...
@Medoy_cmd(pattern="purge( (.*)|$)", manager=True, require="delete_messages")
...
@Medoy_cmd(pattern="pinned", manager=True, groups_only=True)
