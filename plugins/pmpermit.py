...
from Medoy.dB.logusers_db import *
from Medoy.dB.pmpermit_db import *
...
if udB.get_key("PMSETTING"):
    if udB.get_key("AUTOAPPROVE"):
        ...
        async def autoappr(e):
            ...
            try:
                await Medoy-bot.edit_folder(miss.id, folder=0)
        ...
        if not is_approved(user.id) and event.text != UND:
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    if inline_pm:
                        results = await Medoy-bot.inline_query(
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await Medoy-bot(BlockRequest(user.id))
                await Medoy-bot(ReportSpamRequest(peer=user.id))
                await asst.edit_message(
                    int(udB.get_key("LOG_CHANNEL")),
                    _not_approved[user.id],
    ...
    @Medoy_cmd(pattern="(start|stop|clear)archive$", fullsudo=True)
    async def _(e):
    ...
    @Medoy_cmd(pattern="(a|ok)(?: |$)", fullsudo=True)
    ...
    @Medoy_cmd(pattern="(no|tolak)(?: |$)", fullsudo=True)
...
@Medoy_cmd(pattern="block( (.*)|$)", fullsudo=True)
...
@Medoy_cmd(pattern="unblock( (.*)|$)", fullsudo=True)
...
@Medoy_cmd(pattern="listapproved$", owner=True)
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        approve_user(uid)
        try:
            await Medoy-bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user = await Medoy-bot.get_entity(uid)
        except BaseException:
...
async def blck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await Medoy-bot(BlockRequest(uid))
    except BaseException:
        pass
    try:
        user = await kazu_bot.get_entity(uid)
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await Medoy-bot(UnblockRequest(uid))
    except BaseException:
        pass
    try:
        user = await kazu_bot.get_entity(uid)
