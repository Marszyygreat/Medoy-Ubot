...
from Medoy.dB import DEVLIST
from Medoy.dB.gban_mute_db import (
    gban,
    gmute,
    is_gbanned,
    is_gmuted,
...
from Medoy.dB.gcast_blacklist_db import (
    add_gblacklist,
    is_gblacklisted,
    rem_gblacklist,
)
...
@Medoy_cmd(pattern="gpromote( (.*)|$)", fullsudo=True)
async def _(e):
...
@Medoy_cmd(pattern="gdemote( (.*)|$)", fullsudo=True)
async def _(e):
    x = e.pattern_match.group(1).strip()
    Medoy-bot = e.client
    if not x:
    ...
    if user:
        ...
        rank = "Not AdMin"
        c = 0
        async for x in Medoy-bot.iter_dialogs():
            if (
                "group" in key.lower()
                and x.is_group
                or "group" not in key.lower()
...
@Medoy_cmd(pattern="ungban( (.*)|$)", fullsudo=True)
async def _(e):
...
@Medoy_cmd(pattern="gban( (.*)|$)", fullsudo=True)
async def _(e):
...
@Medoy_cmd(pattern="g(admin|)cast( (.*)|$)", fullsudo=False)
async def gcast(event):
    ...
    if err != "":
        ...
        text += f"\Anda dapat melakukan `{HNDLR}Medoy gcast-error.log` untuk mengetahui laporan kesalahan."
    await kk.edit(text)


@Medoy_cmd(pattern="gucast( (.*)|$)", fullsudo=True)
...
@Medoy_cmd(pattern="gkick( (.*)|$)", fullsudo=True)
async def gkick(e):
...
@Medoy_cmd(pattern="gmute( (.*)|$)", fullsudo=True)
async def _(e):
...
@Medoy_cmd(pattern="ungmute( (.*)|$)", fullsudo=True)
async def _(e):
...
async def list_gengbanned(event):
    ...
    if len(gbanned_users) > 4096:
        ...
        await x.reply(
            file="gbanned.txt",
            message=f"List of users GBanned by {inline_mention(Medoy-bot.me)}",
        )
...
@Medoy_cmd(pattern="gblacklist$")
async def blacklist_(event):
    await gblacker(event, "add")


@Medoy_cmd(pattern="ungblacklist$")
