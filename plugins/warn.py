...
from Medoy.dB.warn_db import add_warn, reset_warn, warns
...
async def warn(e):
    ...
    if count + 1 >= number:
        if "ban" in action:
            try:
                await Medoy-bot.edit_permissions(e.chat_id, user, view_messages=False)
            except BaseException:
                return await e.eor("`Ada yang salah.`", time=5)
        elif "kick" in action:
            try:
                await Medoy-bot.kick_participant(e.chat_id, user)
            except BaseException:
                return await e.eor("`Ada yang salah.`", time=5)
        elif "mute" in action:
            try:
                await Medoy-bot.edit_permissions(
        ...
        ok = await Medoy-bot.get_entity(user)
...
@Medoy_cmd(pattern="setwarn( (.*)|$)", manager=True)
async def warnset(e):
