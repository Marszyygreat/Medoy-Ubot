...
from Medoy.dB.notes_db import add_note, get_notes, list_note, rem_note
from Medoy.fns.tools import create_tl_btn, format_btn, get_msg_button

from . import events, get_string, mediainfo, udB, Medoybot, Medoycmd
from ._inline import something


@Medoy_cmd(pattern="addnote( (.*)|$)", admins_only=True)
async def an(e):
    ...
    Medoy-bot.add_handler(notes, events.NewMessage())


@Medoy_cmd(pattern="remnote( (.*)|$)", admins_only=True)
async def rn(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    chat = e.chat_id
    if not wrd:
...
@Medoy_cmd(pattern="listnote$", admins_only=True)
async def lsnote(e):
