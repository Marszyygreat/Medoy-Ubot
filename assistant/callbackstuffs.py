...
try:
    from Medoy.fns.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
...
from Medoy.fns.helper import fast_download, progress
from Medoy.fns.tools import Carbon, async_searcher, get_paste, telegraph_client
from Medoy.startup.loader import Loader
    ...
    if not match:
        try:
            ...
            return await okk.edit(
                f"**• Medoy-Ubot •**{cli}", file=img, buttons=button
            )
        except Exception as er:
            LOGS.exception(er)
