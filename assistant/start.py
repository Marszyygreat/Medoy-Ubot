...
from Medoy._misc import SUDO_M, owner_and_sudos
from Medoy.dB.asst_fns import *
...
**◈ ᴋᴀᴢᴜ ᴜʙᴏᴛ​ ◈ [v{Medoy_version}](https://github.com/Marszyygreat/Medoy-Ubot), powered by @Siid0yyy**
"""
...
async def own(event):
    msg = Owner_info_msg.format(
        mention=event.sender.mention, me=inline_mention(Medoy-bot.me)
    )
    if custom_info:
...
async def ayra(event):
    ...
    if event.sender_id not in SUDO_M.fullsudos:
        ok = ""
        me = inline_mention(Medoy-bot.me)
