...
if sys.argv[0] == "-m":
    ...
    if BOT_MODE:
        ...
        Medoy-bot = MedoyClient(
            validate_session(Var.SESSION, LOGS),
            udB=udB,
            app_version=Medoy_version,
            device_model="Medoy",
        )
        Medoy-bot.run_in_loop(autobot())

    asst = MedoyClient(None, bot_token=udB.get_key("BOT_TOKEN"), udB=udB)

    if BOT_MODE:
        kazu_bot = asst
        if udB.get_key("OWNER_ID"):
            try:
                Medoy-bot.me = Medoy-bot.run_in_loop(
                    Medoy-bot.get_entity(udB.get_key("OWNER_ID"))
                )
            except Exception as er:
                LOGS.exception(er)
    elif not asst.me.bot_inline_placeholder:
       Medoy-bot.run_in_loop(enable_inline(Medoy-bot, asst.me.username))
    ...
    print("Medoy 2022 © Medoy-Ubot")

    from logging import getLogger

    LOGS = getLogger("Medoy")

    Medoy-bot = asst = udB = vcClient = None
