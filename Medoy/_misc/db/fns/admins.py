...
async def _callback_check(event):
    ...
    if not _Medoy_cache.get("admin_callback"):
        _Medoy_cache.update({"admin_callback": {id_: None}})
    else:
        _Medoy_cache["admin_callback"].update({id_: None})
    while not _Medoy_cache["admin_callback"].get(id_):
        await asyncio.sleep(1)
    key = _Medoy_cache.get("admin_callback", {}).get(id_)
    del _Medoy_cache["admin_callback"][id_]
    return key


async def get_update_linked_chat(event):
    if _Medoy_cache.get("LINKED_CHATS") and _kazu_cache["LINKED_CHATS"].get(event.chat_id):
        ...
        if _Medoy_cache.get("LINKED_CHATS"):
            _Medoy_cache["LINKED_CHATS"].update({event.chat_id: {"linked_chat": _ignore}})
        else:
            _Medoy_cache.update(
