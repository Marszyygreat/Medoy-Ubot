...
if run_as_module:
    ...
    from ..version import __version__ as __Medoy__
    from ..version import Medoy_version

    file = f"Medoy{sys.argv[6]}.log" if len(sys.argv) > 6 else "Medoy.log"
    ...
    LOGS.info(f"py-Medoy Version - {__Medoy__}")
    LOGS.info(f"Telethon Version - {__version__} [Layer: {LAYER}]")
    LOGS.info(f"Medoy Version - {kazu_version} [{HOSTED_ON}]")
