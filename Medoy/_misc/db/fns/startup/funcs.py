...
async def join_ajg():
    ...
    try:
        await Medoy-bot(JoinChannelRequest("KegabutanDoy"))
        await Medoy-bot(JoinChannelRequest("mutualanonlyone"))
...
async def startup_stuff():
    ...
    if CT := udB.get_key("CUSTOM_THUMBNAIL"):
        ...
        MedoyConfig.thumb = path
        try:
            await download_file(CT, path)
        except Exception as er:
            LOGS.exception(er)
    elif CT is False:
        MedoyConfig.thumb = None
...
async def autobot():
    from .. import udB, Medoy-bot

    if udB.get_key("BOT_TOKEN"):
        return
    await Medoy-bot.start()
    ...
    if who.username:
        ...
        username = f"Medoy-{str(who.id)[5:]}-bot"
    bf = "@BotFather"
    await kazu_bot(UnblockRequest(bf))
    await Medoy-bot.send_message(bf, "/cancel")
    await asyncio.sleep(1)
    await Medoy-bot.send_message(bf, "/newbot")
    await asyncio.sleep(1)
    isdone = (await Medoy-bot.get_messages(bf, limit=1))[0].text
    ...
    if not isdone.startswith("Good."):
        await Medoy-bot.send_message(bf, "My Assistant Bot")
        await asyncio.sleep(1)
        isdone = (await Medoy-bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.critical(
            ...
            sys.exit(1)
    await Medoy-bot.send_message(bf, username)
    await asyncio.sleep(1)
    isdone = (await Medoy-bot.get_messages(bf, limit=1))[0].text
    await Medoy-bot.send_read_acknowledge("botfather")
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = f"Medoy-{str(who.id)[6:]}{ran}-bot"
        await Medoy-bot.send_message(bf, username)
        await asyncio.sleep(1)
        isdone = (await Medoy-bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("Done!"):
        ...
        await enable_inline(Medoy-bot, username)
        LOGS.info(
...
async def autopilot():
    from .. import asst, udB, Medoybot

    channel = udB.get_key("LOG_CHANNEL")
    new_channel = None
    if channel:
        try:
            chat = await Medoy-bot.get_entity(channel)
        ...
        try:
            r = await kazu_bot(
                CreateChannelRequest(
                    title="Logs MEDOY-Ubot",
                    about="Logs Medoy-Ubot \n\n Cʀᴇᴀᴛᴇᴅ Bʏ @KegabutanDoy",
                    megagroup=True,
                ),
            )
    ...
    assistant = True
    try:
        await Medoy-bot.get_permissions(int(channel), asst.me.username)
    except UserNotParticipantError:
        try:
            await Medoy-bot(InviteToChannelRequest(int(channel), [asst.me.username]))
        except BaseException as er:
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        ...
        ll = await Medoy-bot.upload_file(photo)
        try:
            await Medoy-bot(
...
async def customize():
    ...
    try:
        ...
        if not Medoybot.me.username:
            sir = Medoy-bot.me.first_name
        else:
            sir = f"@{Medoy-bot.me.username}"
        file = random.choice(
            [
                "https://graph.org//file/d854abd533a783c6642b1.jpg",
                "resources/extras/logo.jpg",
        ...
        await asyncio.sleep(1)
        await Medoy-bot.send_message("botfather", "/cancel")
        await asyncio.sleep(1)
        await Medoy-bot.send_message("botfather", "/setuserpic")
        await asyncio.sleep(1)
        isdone = (await Medoybot.get_messages("botfather", limit=1))[0].text
        if isdone.startswith("Invalid bot"):
            LOGS.info("Error while trying to customise assistant, skipping...")
            return
        await Medoy-bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await Medoy-bot.send_file("botfather", file)
        await asyncio.sleep(2)
        await Medoy-bot.send_message("botfather", "/setabouttext")
        await asyncio.sleep(1)
        await Medoy-bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await Medoy-bot.send_message(
            "botfather", f"✨ Hello ✨!! I'm Assistant Bot of {sir}"
        )
        await asyncio.sleep(2)
        await Medoy-bot.send_message("botfather", "/setdescription")
        await asyncio.sleep(1)
        await Medoy-bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await Medoy-bot.send_message(
            "botfather",
            f"✨ Powerful Medoy Assistant Bot ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @mutualanonlyone ✨",
        )
        await asyncio.sleep(2)
...
async def ready():
    from .. import asst, udB, Medoy-bot
    from ..fns.tools import async_searcher

    chat_id = udB.get_key("LOG_CHANNEL")
    spam_sent = None
    if not udB.get_key("INIT_DEPLOY"):  # Detailed Message at Initial Deploy
        MSG = """ **Thanks for Deploying Medoy-Ubot!**
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = "https://graph.org//file/d854abd533a783c6642b1.jpg"
        BTTS = Button.inline("• Click to Start •", "initft_2")
        udB.set_key("INIT_DEPLOY", "Done")
    else:
        MSG = f"**Medoy-Ubot has been deployed!**\n➖➖➖➖➖➖➖➖➖➖\n**UserMode**: {inline_mention(Medoy-bot.me)}\n**Assistant**: @{asst.me.username}\n➖➖➖➖➖➖➖➖➖➖\n**Support**: @mutualanonlyonep\n➖➖➖➖➖➖➖➖➖➖"
...
async def WasItRestart(udb):
    ...
    if not key:
        return
    from .. import asst, Medoy-bot

    try:
        data = key.split("_")
        who = asst if data[0] == "bot" else Medoy-bot
...
async def enable_inline(Medoy-bot, username):
    bf = "BotFather"
    await Medoy-bot.send_message(bf, "/setinline")
    await asyncio.sleep(1)
    await Medoy-bot.send_message(bf, f"@{username}")
    await asyncio.sleep(1)
    await Medoy-bot.send_message(bf, "Search")
    await Medoy-bot.send_read_acknowledge(bf)
