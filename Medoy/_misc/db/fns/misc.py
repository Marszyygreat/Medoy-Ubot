...
async def allcmds(event, telegraph):
    ...
    await eor(event, f"All Medoy Cmds : [Click Here]({t['url']})", link_preview=False)
...
async def _insta_login():
    if "insta_creds" in kazu_bot._cache:
        return Medoy-bot._cache["insta_creds"]
...
async def get_insta_code(username, choice):
    ...
    async with asst.conversation(Medoy-bot.uid, timeout=60 * 2) as conv:
