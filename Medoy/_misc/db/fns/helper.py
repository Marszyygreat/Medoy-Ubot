...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "©↻ꝛɪᴢ"
            await eod(ok, f"✓ `Medoy - Installed`: `{plug}` ✓\n\n{output}")
    ...
    async def heroku_logs(event):
        ...
        try:
            app = (heroku22.from_key(Var.HEROKU_API)).app(Var.HEROKU_APP_NAME)
        ...
        ok = app.get_log()
        with open("mEDOY-heroku.log", "w") as log:
            log.write(ok)
        await event.client.send_file(
            event.chat_id,
            file="MEDOY-heroku.log",
            thumb=MedoyConfig.thumb,
            caption="**Medoy Heroku Logs.**",
        )

        os.remove("MARSZYY_USERBOT-heroku.log")
        await xx.delete()
...
async def restart(ay=None):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        try:
            Heroku = heroku22.from_key(Var.HEROKU_API)
