    ...
    def decor(dec):
        async def wrapp(ay):
            ...
            try:
                ...
                await Medoy-bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await Medoy-bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        Button.url("Bot", "t.me/Medoy-stringbot?start="),
                        Button.url(
                            "Repl",
                            "https://replit.com/@Medoy/MedoyStringSession",
                        ),
                    ],
                )
                sys.exit()
                ...
                ftext = "**Medoy Client Error:** `Forward this to` @mutualanonlyone\n\n"
                ftext += f"**Medoy Version:** `{str(pyver)}"
                ftext += "`\n**Userbot Version:** `" + str(Medoy_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START Medoy CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**Kazu Client Error:** `Forward this to` @KegabutanDoy\n\n",
                        )
                else:
